  ...
  <form class="meetup-form" @submit.prevent="handleSubmit">
    <div class="meetup-form__content">
      ...
      <meetup-agenda-item-form
        ...
        @update:agenda-item="updateAgendaItem($event, index)"
      />
...
export default {
  ...
  methods: {
    handleSubmit() {
      const submittedMeetup = cloneDeep(this.localMeetup);

      if (submittedMeetup.imageToUpload) {
        submittedMeetup.image = submittedMeetup.imageToUpload;
        delete submittedMeetup.imageToUpload;
      }

      this.$emit('submit', submittedMeetup);
    },
    ...
    updateAgendaItem(event, index) {
      this.localMeetup.agenda.splice(index, 1, event);
    },
